...
bool isSafe(int newX, int newY, vector<vector<int> > &temp){
    ...
    int orangesRotting(vector<vector<int>>& grid) {
        ...
        for(int i=0; i<n; i++){
            for(int j=0; j<m; j++){
                if(temp[i][j] == 2){
                    //every src node ka time set kar rahe hain
                    //index of all rotten oranges
                    q.push({{i,j}, 0});
                }
            }
        }
        ...
        //we checked all rotten oranges till now
        //now i want to check all oranges are rotten or not
        for(int i=0; i<n; ++i){
            for(int j=0; j<m; ++j){
